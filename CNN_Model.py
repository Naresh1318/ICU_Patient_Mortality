...
for f_s in top_feat_no:
    ...
    if f_s == 726:
        X_test2_sd = np.reshape(X_test2_sd, (X_test2_sd.shape[0], 1, 33, 22))
        X_train2_sd = np.reshape(X_train2_sd, (X_train2_sd.shape[0], 1, 33, 22))
    else:
        X_test2_sd = np.reshape(X_test2_sd, (X_test2_sd.shape[0], 1, int(np.sqrt(f_s)), int(np.sqrt(f_s))))
        X_train2_sd = np.reshape(X_train2_sd, (X_train2_sd.shape[0], 1, int(np.sqrt(f_s)), int(np.sqrt(f_s))))
    ...
    if f_s == 726:
        ...
        model.add(Convolution2D(64, 3, 3, border_mode='same', input_shape=(1, int(np.sqrt(f_s)), int(np.sqrt(f_s)))))
...
plt.ylim([90.0, 95.0])
plt.title('{} Feature Selection'.format('Convolutional neural network'))
